...
SC_train = zeros(size(train_data, 3), nBinsR, nBinsTheta, 50);
...
for it = 1:size(test_data, 3)
    ...
    HC = zeros(1, size(SC_train, 1));
    for it_train = 1:size(SC_train, 1)
        HC(it_train) = histCost(SC, squeeze(SC_train(it_train, :, :, :)));
    end
